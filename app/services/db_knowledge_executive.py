...
DimProduct  — Product master data for inventory and sales analysis.
  ...
  Construction         (Construction method or technique)
  ImagePath            (Path to product image)
  MinOrderQty          (Minimum order quantity)
  CollectionType       (Collection type classification)
...
DimWarehouse  — Warehouse locations.
  WarehouseKey         (Surrogate warehouse key, PK)
  WarehouseID          (Business location identifier)
  WarehouseName        (Descriptive name of the warehouse)
  City                 (City)
  State                (State)
  Country              (Country)
  ZIP                  (Postal code)
...
DimBillOfLading  — Bill of lading header for shipment tracking.
  BillOfLadingKey      (Surrogate key, PK)
  BillOfLadingNo       (Bill of lading number)
  ...
  ShipVia              (Ship via)
  ScacCode             (SCAC code)
  PickUpDateKey        (FK → DimDate — pickup date)
  DeliveryDateKey      (FK → DimDate — delivery date)
  TotalAmount          (Total amount)
  Freight              (Freight amount)
  CODAmount            (COD amount)
  CODChargesPrepaid    (COD charges prepaid)
  CarrierProNo         (Carrier pro number)
  Status               (Status)
  ADDDateKey           (FK → DimDate — add date)

DimBillOfLadingDetail  — Bill of lading line-level detail.
  BillOfLadingDetailKey (Surrogate key, PK)
  BillOfLadingNo       (Bill of lading number)
...
DimRugOAK  — Rug OAK inventory master data.
  RugKey               (Surrogate key, PK)
  SKU                  (Stock ID)
  ItemID               (Item ID)
  RugID                (Rug ID)
  ...
  SQFTPrice            (Price per square foot)
...
DimItemPrice  — Item pricing by price category.
  ItemPriceKey         (Surrogate key, PK)
  ItemID               (Item ID)
  ...
  AddUser              (User who added the record)
  AddDate              (Date record was added)
  MODUser              (User who last modified the record)
  MODDate              (Date of last modification)

DimRegion  — Geographic region master data.
  RegionKey            (Surrogate key, PK)
...
DimGLAccount  — General ledger account master data.
  GLAccountKey         (Surrogate key, PK)
...
FactSalesInvoice  — Invoice header metrics.
  ...
  WarehouseKey         (FK → DimWarehouse — fulfillment warehouse)
...
FactSalesDetail  — Invoice line-level sales details.
  ...
  ReturnQuantity       (Sale item quantity returned after sale)
...
FactConsignments  — Consignment header metrics.
  ...
  ConsignmentNo        (Unique consignment number)
  DateKey              (FK → DimDate — consignment date)
  CustomerKey          (FK → DimCustomer)
  AddressesKey         (FK → DimInvoiceAddresses — shipping destination)
  SalesType            (Sales type classification)
  InvoiceType          (Invoice category)
  Status               (Current consignment status)
  TotalQuantity        (Total quantity of items)
  TotalAmount          (Total monetary value including all charges)
  TaxAmount            (Total tax amount)
  ShippingCharges      (Shipping and transportation charges)
  HandlingCharges      (Handling, packaging, or processing charges)
  ServiceCharges       (Additional service-related fees)
  MerchandiseAmount    (Value of goods only — excluding services and charges)
  ServicesAmount       (Value of services included)
  AppliedAmount        (Amount applied)
  DiscountAmount       (Total discount applied)
  TotalWeight          (Total weight)

FactConsignmentDetail  — Consignment line-level details.
  ConsignmentDetailKey (Surrogate key, PK)
  ConsignmentNo        (Links to consignment header — FactConsignments)
  ...
  DiscountAmount       (Discount applied to this line)
  SalesAmount          (Total sales amount — Quantity × UnitPrice − Discount)
  TaxAmount            (Tax amount for this line)
  ShippingCharges      (Shipping cost allocated to this line)
  ProfitAmount         (Profit earned on this line)
...
FactPurchaseOrder  — Purchase order header metrics.
  ...
  POStatus             (Purchase order status — Open, Closed, Partially Shipped, Void, New, Cancel)
...
FactInventorySnapshot  — Inventory position snapshots.
  ...
  OriginalCost         (Inventory original cost)
...
FactVendorInvoices  — Vendor position invoices.
  ...
  VendorInvoiceDateKey (FK → DimDate — vendor invoice date)
  DueDateKey           (FK → DimDate — due date)
  DiscountDateKey      (FK → DimDate — discount date)
  CutOffDateKey        (FK → DimDate — cut-off date)
  TotalAmount          (Total invoice amount before payments and adjustments)
  PaidAmount           (Amount already paid to the vendor)
  DiscountAmount       (Early payment discount offered by the vendor)
...
FactVendorPayments  — Vendor position payments.
...
FactCustomerPayment  — Customer payments (incoming cash receipts).
  ...
  Status               (Payment status)
  Approved             (Flag indicating if payment is approved)
  PeriodID             (Accounting period identifier)
...
FactCustomerDebit  — Summary level data for customer credit memos and debit adjustments.
  ...
  WarehouseKey         (FK → DimWarehouse — which location issued the credit)
  ...
  TotalQty             (Total quantity of items being credited/debited)
  TotalQtyInvoiced     (Total quantity originally invoiced)
  TotalMerchandise     (Value of physical goods being credited)
...
FactVendorReturn  — Summary level data for vendor returns.
  ...
  PaymentAccount       (Account code where return payment is recorded)
  OtherChargesAccount  (Account for freight, handling, restocking, or other charges)
  PeriodID             (Accounting period identifier)
...
FactSalesOrders  — Sales order header summary.
  SalesOrderKey        (Surrogate key, PK)
  SalesOrderNo         (Sales order number)
  CustomerKey          (FK → DimCustomer)
  WarehouseKey         (FK → DimWarehouse)
  PriceCategoryKey     (FK → DimPriceCategory)
  PaymentTermKey       (FK → DimPaymentTerms)
  OrderDateKey         (FK → DimDate — order date)
  ShippingDateKey      (FK → DimDate — shipping date)
  CancelDateKey        (FK → DimDate — cancel date)
  SalesType            (Sales order sales type)
  Status               (Sales order status)
  SpecialOrder         (Special order flag)
  TotalQty             (Sales order total quantity)
  TotalQtyShipped      (Sales order total quantity shipped)
  TotalMerchandise     (Sales order total merchandise amount)
  TotalServices        (Sales order total service charges)
  TotalAmount          (Sales order total amount)
  TaxAmount            (Sales order tax amount)
  ServiceCharges       (Sales order service charges)
  ShippingCharges      (Sales order shipping charges)
  SalesDiscount        (Sales order sales discount)
  OpenCredit           (Sales order remaining balance of the credit)
  PeriodID             (Sales order period ID)
  PaymentDiscount      (Sales order payment discount)

FactSalesOrderDetail  — Sales order line-level detail.
  SalesOrderDetailKey  (Surrogate key, PK)
  SalesOrderNo         (Sales order number)
  CustomerKey          (FK → DimCustomer)
  ProductKey           (FK → DimProduct)
  WarehouseKey         (FK → DimWarehouse)
  PriceCategoryKey     (FK → DimPriceCategory)
  OrderDateKey         (FK → DimDate — order date)
  RequiredDateKey      (FK → DimDate — required date)
  ShippingDateKey      (FK → DimDate — shipped date)
  ItemType             (Item type)
  SKU                  (Stock ID)
  OrderQty             (Order quantity)
  ToShippedQty         (To-ship quantity)
...
FactPackingSlips  — Packing slip header summary.
  PackingSlipKey       (Surrogate key, PK)
  PackingSlipNo        (Packing slip number)
  CustomerKey          (FK → DimCustomer)
  SalesOrderNo         (Packing slip order number)
  WarehouseKey         (FK → DimWarehouse)
  DateCreatedKey       (FK → DimDate — date created)
  DatePrintedKey       (FK → DimDate — date printed)
  SalesInvoiceNo       (Packing slip sales invoice number)
  SalesType            (Sales type)
  Status               (Packing slip status)
  TotalQty             (Total quantity)
  TotalBales           (Total bales)
  TotalWeight          (Total weight)
  TotalSQFT            (Total square footage)
  TotalCuft            (Total cubic feet)
  ShippingCharges      (Total shipping charges)
  HandlingCharges      (Handling charges)
  PeriodID             (Period ID)

FactPackingSlipDetail  — Packing slip line-level detail.
  PackingSlipDetailKey (Surrogate key, PK)
  PackingSlipNo        (Packing slip number)
  Line_No              (Line number)
  ProductKey           (FK → DimProduct)
  WarehouseKey         (FK → DimWarehouse)
...
FactVendorPackingSlips  — Vendor packing slip header summary.
  VPackingSlipKey      (Surrogate key, PK)
  VPackingSlipNo       (Vendor packing slip number)
  VendorKey            (FK → DimVendors)
  PurchaseOrderNo      (Purchase order number)
  DateShippedKey       (FK → DimDate — date shipped)
  ArrivalDateKey       (FK → DimDate — arrival date)
  VPackingSlipDateKey  (FK → DimDate — packing slip date)
  TotalQuantity        (Total quantity)
  TotalAmount          (Total amount)
...
FactVendorPackingSlipDetail  — Vendor packing slip line-level detail.
  VPackingSlipDetailKey (Surrogate key, PK)
  VPackingSlipNo       (Vendor packing slip number)
  Line_No              (Line number)
  ProductKey           (FK → DimProduct)
  VendorKey            (FK → DimVendors)
...
FactSalescommission  — Sales commission summary.
  CommissionKey        (Surrogate key, PK)
  CommissionNo         (Commission number)
  SalesInvoiceNo       (Sales invoice number)
  SalesOrderNo         (Sales order number)
  SalesRepKey          (FK → DimSalesRep)
  CustomerIDKey        (FK → DimCustomer)
  PoolID               (Commission pool ID)
  ...
  TotalAmount          (Total amount)
  AmountForCommission  (Amount for commission)
  PaidAmount           (Paid amount)
  CDS                  (CDS flag)
  MCE                  (MCE flag)
  InvoiceDateKey       (FK → DimDate — invoice date)
  DateKey              (FK → DimDate)
...
FactCommissionInvoice  — Sales Rep commission invoices.
  ...
  CommissionInvoiceDateKey (FK → DimDate — commission invoice date)
  InvoiceDateKey       (FK → DimDate — invoice date)
  DueDateKey           (FK → DimDate — due date)
...
FactCommissionRate  — Commission rate definitions per sales rep and customer.
  CommissionRateKey    (Surrogate key, PK)
  SalesRepKey          (FK → DimSalesRep)
  CustomerKey          (FK → DimCustomer)
  ...
  CDS                  (CDS flag)
  SalesRepCompany      (Sales rep company)
  Line_No              (Line number)
  MasterAgent          (Master agent flag)
  CollectionType       (Collection type)

FactAccountMonthlySummary  — Monthly GL account balance summary.
  AccountMonthlyKey    (Surrogate key, PK — unique identifier for each monthly record)
  GLAccountKey         (FK → DimGLAccount)
  DateKey              (FK → DimDate)
  AccountID            (Business-level account code or number)
  Description          (Name or description of the account)
  PeriodStartDate      (Starting date of the accounting period)
  OpeningBalance       (Balance at the beginning of the period)
  PTD_Debit            (Total debit transactions for the period)
  PTD_Credit           (Total credit transactions for the period)
  PTD_Net              (Net movement during the period)
  YTD_Debit            (Total debits from the start of the year)
  YTD_Credit           (Total credits from the start of the year)
  YTD_Net              (Net movement for the year)
  ClosingBalance       (Ending balance of the account for the period)
